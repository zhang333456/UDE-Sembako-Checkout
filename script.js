...
function renderProductsByCategory() {
  ...
  categories.forEach(cat => {
    ...
    products
      .filter(p => p.active && (p.category || "Umum") === cat)
      .forEach(p => {
...
function changeQty(id, delta) {
  ...
  document.getElementById("total-" + id).innerText =
    cart[id]
      ? `Rp ${(cart[id].price * cart[id].qty).toLocaleString()}`
      : "Rp 0";
...
function updateSummary() {
  ...
  for (let id in cart) {
    ...
    miniCard.innerHTML = `
      ...
      Rp ${(item.price * item.qty).toLocaleString()}</span>
  ...
  if (totalItems === 0)
    cartItemsDiv.innerHTML = "Belum ada barang";
  ...
  if (area === "gadut") shipping = 0;
    ...
    (area === "bukittinggi" || area === "tilatang kamang")
    && totalItems > 1
  ) shipping = 0;
  else if (
    area === "bukittinggi" || area === "tilatang kamang"
  ...
  document.getElementById("summary").innerHTML = `
    ...
    Rp ${total.toLocaleString()}</span><br>

    Ongkir:
    <span style="font-size:14px;
    color:${shipping === 0 ? "teal" : "#333"}">
    ${shipping === 0
      ? "GRATIS ONGKIR"
    ...
    Rp ${(total + shipping).toLocaleString()}
    </span>
  `;

  document.getElementById("floatingCheckout").style.display =
...
function scrollToCart() {
  ...
  document.getElementById("floatingCheckout")
    .style.display = "none";
}
...
function checkout() {
  ...
  let nama =
    document.getElementById("namaPemesan").value.trim();
  ...
  let area = document.getElementById("area").value;
  let alamat =
    document.getElementById("detailAlamat").value;
  ...
  let metode =
    document.querySelector('input[name="bayar"]:checked');
